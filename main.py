...
from utils.feishu_notifier import FeishuNotifier   # ← 新增
        ...
        try:
            ...
            try:
                if message['3']['redReminder'] == '等待买家付款':
                    ...
                    user_id = message['1'].split('@')[0]
                    logger.info(f'交易成功 {user_id} 等待卖家发货')
                    return
            except Exception:
                pass
            ...
            if send_user_id == self.myid:
                ...
                if self.check_toggle_keywords(send_message):
                    ...
                    if mode == "manual":
                        ...
                        # ↓ 飞书通知：切换为人工接管
                        self.feishu.notify_manual_mode_change(chat_id, "manual", item_id)
                    else:
                        logger.info(f"🟢 已恢复会话 {chat_id} 的自动回复 (商品: {item_id})")
                        # ↓ 飞书通知：恢复 AI
                        self.feishu.notify_manual_mode_change(chat_id, "auto", item_id)
                    return
            ...
            # ↓ 飞书通知：买家新消息（在 AI 回复之前发出，确保实时性）
            self.feishu.notify_new_message(
                customer_name=send_user_name,
                customer_id=send_user_id,
